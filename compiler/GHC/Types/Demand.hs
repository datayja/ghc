...
module GHC.Types.Demand (
    ...
    -- ** Algebra
    absDmd, topDmd, botDmd, seqDmd, topSubDmd, topUbxSubDmd,
    -- *** Least upper bound
...
As far as the permission to unbox is concerned, *evaluatedness* of the argument is
the important trait. Unboxing implies eager evaluation of an argument and we don't
want to change the termination properties of the function. One way to ensure that
is to unbox strict arguments only, but strictness is only a sufficient condition
for evaluatedness.
...
(as Note [Strictness and Unboxing] might suggest). Here's an example:
```
...
the box, we make Boxed win over any Unboxed uses. See Note [Boxed should win]
for the reasoning. Thus `lub Boxed Unboxed = Boxed`.
...
Boxity analysis as described is a bit too conservative for recursive functions,
though. Consider
```
sum z      []          = z
sum (I# n) ((I# x):xs) = sum (I# (n +# x)) xs
```
We really want to unbox 'z', because then the box in the recursive call site
can cancel away and we get a tight, non-allocating recursive case at the cost
of allocating a single box in the base case (that will ultimately cancel away
due to CPR analysis). But 'z' appears Boxed in the base case and
Note [Boxed should win]!
Our current hack is to analyse recursive function bodies with an Unboxed demand,
meaning we pretend that the result is used Unboxed, as described in
Note [Unboxed demands on recursive function bodies] in "GHC.Core.Opt.DmdAnal".

Note [Boxed should win]
~~~~~~~~~~~~~~~~~~~~~~~
Both in 'lubDmd' and 'plusDmd' we want 'Boxed' to win over Unboxed.
Why? Because 'Boxed' carries the implication the whole thing is used, box and
all, so we don't want to unbox it in w/w.
If we use it both boxed and unboxed, then we are definitely using the box,
and so we are quite likely to pay a reboxing cost. So we make Boxed win here.

Example is in the Buffer argument of GHC.IO.Handle.Internals.writeCharBuffer

Some measurements from pre-2013, but boxity analysis worked quite different back
then:

Baseline: (A) Not making Boxed win (Unboxed wins)
Compare with: (B) making Boxed win for lub and both

            Min          -0.3%     -5.6%    -10.7%    -11.0%    -33.3%
            Max          +0.3%    +45.6%    +11.5%    +11.5%     +6.9%
 Geometric Mean          -0.0%     +0.5%     +0.3%     +0.2%     -0.8%

Baseline: (B) Making Boxed win for both lub and both
Compare with: (C) making Boxed win for plus, but Unboxed win for lub
...
lubBoxity Unboxed Unboxed = Unboxed
lubBoxity _       !_      = Boxed -- See Note [Boxed should win]

plusBoxity :: Boxity -> Boxity -> Boxity
-- Boxity is a syntactic property, so the semantic distinction of lub vs. plus
-- is irrelevant. If we use a box somewhere in the program, then we better not
-- unbox! See also Note [Boxed should win].
plusBoxity = lubBoxity
...
data SubDemand
  ...
  -- or @Call n (Poly Boxed n)@. 'mkCall' and 'mkProd' do these rewrites.
  --
...
topSubDmd, topUbxSubDmd, botSubDmd, seqSubDmd :: SubDemand
topSubDmd    = Poly   Boxed C_0N
topUbxSubDmd = Poly Unboxed C_0N
...
-- equality @Prod b [polyFieldDmd n, ...] === Poly b n@, simplifying to 'Poly'
-- 'SubDemand's when possible.
mkProd :: Boxity -> [Demand] -> SubDemand
-- We only want to simplify absent and bottom demands and unbox the others.
-- See also Note [Boxed should win] and Note [Don't optimise LP(L,L,...) to L].
mkProd b ds
  | all (== AbsDmd) ds = Poly b C_00
  | all (== BotDmd) ds = Poly b C_10
  | dmd@(n :* Poly Boxed m):_ <- ds
  , n == m
  , all (== dmd) ds    = Poly b n
  | otherwise          = Prod b ds
...
polyFieldDmd n    = n :* Poly Boxed n & assertPpr (isCardNonOnce n) (ppr n)
...
unboxSubDemand (Prod _ ds) = Prod Unboxed ds
...
lubSubDmd (Prod b1 ds1) (Poly b2 n2)
  = Prod (lubBoxity b1 b2) (strictMap (lubDmd (polyFieldDmd n2)) ds1)
lubSubDmd (Prod b1 ds1) (Prod b2 ds2)
  | equalLength ds1 ds2
  = Prod (lubBoxity b1 b2) (strictZipWith lubDmd ds1 ds2)
-- Handle Call
...
plusSubDmd (Prod b1 ds1) (Poly b2 n2)
  = Prod (plusBoxity b1 b2) (strictMap (plusDmd (polyFieldDmd n2)) ds1)
plusSubDmd (Prod b1 ds1) (Prod b2 ds2)
  | equalLength ds1 ds2
  = Prod (plusBoxity b1 b2) (strictZipWith plusDmd ds1 ds2)
-- Handle Call
...
multSubDmd n    (Prod b ds)  = Prod b (strictMap (multDmd n) ds)
...
isSeqDmd (C_11 :* Poly _ C_00) = True
isSeqDmd (C_1N :* Poly _ C_00) = True
isSeqDmd _                     = False
...
strictifyDictDmd ty (n :* Prod b ds)
  ...
  = C_1N :* Prod b (zipWith strictifyDictDmd field_tys ds)
...
addCaseBndrDmd sd alt_dmds
  ...
  -- No longer ignoring _b is subject of !4969
  | Prod _b ds <- plusSubDmd sd (Prod Unboxed alt_dmds) = ds
  | otherwise                                           = alt_dmds
...
The reason is being able to express 'seq' as a 'Demand', as `1!A`, which is the
pretty-print of `C_11 :* Poly Unboxed C_00`. If you `lubDmd` `1!A` with `A`,
you'll get `L!A` and we generally don't want to keep the Unboxed flag on a lazy
demand (cf. Note [No lazy, Unboxed demands in demand signature]),
so we turn that into `LA` in the demand signature, so there we need
`Poly Boxed C_00`. See Note [Finalising boxity for demand signature].

Even if `Poly Boxed C_00` wasn't possible, it still seems cleaner to simply
store a Boxity flag with each 'Poly' rather than to say that all absent 'Poly's
are Unboxed, while the other 'Poly's (i.e., L, S) are Boxed.
...
dmdTransformDictSelSig (DmdSig (DmdType _ [_ :* prod] _)) call_sd
   ...
   , Prod _ sig_ds <- prod
   = multDmdType n $
     DmdType emptyDmdEnv [C_11 :* Prod Unboxed (map (enhance sd') sig_ds)] topDiv
   | otherwise
...
kill_usage_sd kfs (Prod b ds) = Prod b (map (kill_usage kfs) ds)
...
trimToType (n :* sd) ts
  ...
  where
    go (Prod b ds) (TsProd tss)
      | equalLength ds tss    = Prod b (zipWith trimToType ds tss)
...
trimBoxity (n :* sd) = n :* go sd
  where
    ...
    go (Prod _ ds) = Prod Boxed (map trimBoxity ds)
